...
<div class="categories">
   ...
   <div class="small-container">
     <div class="row row-2">
        ...
        <option value="prodotti.php">Visualizza tutti</option>
        <option value="prodotti.php?filter=<50">Fino a 50€</option>
        <option value="prodotti.php?filter=<100">Da 50€ a 100€</option>
        <option value="prodotti.php?filter=<200">Da 100€ a 200€</option>
        <option value="prodotti.php?filter=<300">Da 200€ a 300€</option>
        <option value="prodotti.php?filter=>300">300€ e oltre</option>
    </select>
     </div>
      <div class="row">
         ...
         $sql = "SELECT articoli.Titolo, articoli.Id, Prezzo, Sconto, Pezzi FROM articoli ";
         if (isset($_GET["filter"])) {
            switch ($_GET["filter"]) {
                case "<50":
                    $sql .= "WHERE (Prezzo * (100 - Sconto) / 100) < 50";
                    break;
                case "<100":
                    $sql .= "WHERE (Prezzo * (100 - Sconto) / 100) > 50 AND (Prezzo * (100 - Sconto) / 100) < 100";
                    break;
                    case "<200":
                     $sql .= "WHERE (Prezzo * (100 - Sconto) / 100) > 100 AND (Prezzo * (100 - Sconto) / 100) < 200";
                     break;
                     case "<300":
                        $sql .= "WHERE (Prezzo * (100 - Sconto) / 100) > 200 AND (Prezzo * (100 - Sconto) / 100) < 300";
                        break;
                case ">300":
                    $sql .= "WHERE (Prezzo * (100 - Sconto) / 100) > 300";
                    break;
                default:
                    $sql .= "WHERE Titolo LIKE '%" . $_GET["filter"] . "%'";
                    break;
            }
         }
          if (isset($_GET["p"]))
